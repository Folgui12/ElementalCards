...
public class AlgDijkstra
{
    ...
    public void Dijkstra(GrafoMA grafo, int source)
    {
        ...
        distance[source] = 0;
        nodos1[source] = nodos2[source] = grafo.nodosID[source];
        ...
        for (int count = 0; count < verticesCount - 1; ++count)
        {
            ...
            for (int v = 0; v < verticesCount; ++v)
            {
                ...
                if (!shortestPathTreeSet[v] && Convert.ToBoolean(graph[u, v]) && distance[u] != int.MaxValue && distance[u] + graph[u, v] < distance[v])
                {
                    ...
                    nodos1[v] = grafo.nodosID[u];
                    nodos2[v] = grafo.nodosID[v];
                }   
            }
        }

        // construyo camino de nodos
        nodos = new int[verticesCount];
        int nodOrig = grafo.nodosID[source];
        for (int i = 0; i < verticesCount; i++)
        {
            if (nodos1[i] != -1)
            {
